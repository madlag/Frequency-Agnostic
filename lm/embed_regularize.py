...
def embedded_dropout_old(embed, sigma, words, dropout=0.1, scale=None, is_training=False):
...
def embedded_dropout(embed, sigma, words, dropout=0.1, scale=None, is_training=False):
  ...
  X = torch.nn.functional.embedding(words, embed.weight,
    padding_idx, embed.max_norm, embed.norm_type,
    embed.scale_grad_by_freq, embed.sparse
  )

  mask = torch.zeros((X.shape[0], 1, 1)).cuda()
