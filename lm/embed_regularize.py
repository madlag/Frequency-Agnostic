...
def embedded_dropout_old(embed, X, sigma, words, dropout=0.1, scale=None, is_training=False):
  ...
  if sigma is None:
    sigma = torch.ones_like(embed.weight)
...
def embedded_dropout(X, sigma, dropout=0.1, scale=None, is_training=False):
  #sigma = torch.where(torch.exp(sigma.weight) > 0.3, torch.ones_like(sigma.weight) * 0.3, torch.exp(sigma.weight))
  #m = torch.distributions.normal.Normal(torch.zeros_like(sigma), torch.ones_like(sigma) * 1)
  #sigma = m.sample() * sigma
  #binary_mask = torch.distributions.bernoulli.Bernoulli(torch.ones(embed.weight.size(0)) * 0.75).sample().cuda()
  ...
  assert(sigma is None) # Means sigma is only ones

  mask = torch.zeros((X.shape[0], 1, 1))
  if torch.cuda.is_available():
    mask = mask.cuda()
  mask = mask.bernoulli_(1 - dropout)
  mask = mask.expand_as(X) / (1 - dropout)
